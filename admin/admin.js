...
async function initAdminApp() {
    ...
    try {
        ...
        // 🔐 التحقق من المصادقة الثنائية (2FA) قبل الدخول للوحة التحكم
        const verified2FA = sessionStorage.getItem('2fa_verified');
        if (!verified2FA) {
            // التحقق من Firestore إذا كان 2FA مفعلاً
            const db = window.firebaseModules.getFirestore(window.firebaseApp);
            const userDoc = await window.firebaseModules.getDoc(window.firebaseModules.doc(db, "users", userData.uid));
            ...
            if (userDoc.exists() && userDoc.data().twoFactorEnabled) {
                if (window.twoFactorAuth) {
                    ...
                    if (!isVerified) {
                        showToast('فشل التحقق الثنائي', 'error');
